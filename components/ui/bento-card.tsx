...
export function BentoCard({ className, delay = 0, glass = false, children, ...props }: BentoCardProps) {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "relative flex flex-col overflow-hidden rounded-[24px] bg-[#09080C] border border-white/[0.08] transition-all duration-300 shadow-2xl",
        glass && "bg-white/[0.02] backdrop-blur-3xl border-white/10",
        className
      )}
      {...props}
    >
